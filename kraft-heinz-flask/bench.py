...
class FeatureInstance:
    ...
    def get_check_weigher(self):
        ...
        # Read correct file
        df_check = pd.read_csv(os.path.join(cn.COMBINED_GRANULAR_DATA_PATH, files[lines.index(self.line.split(" ")[1])]))
        ...
        if not self.training:
            ...
            if self.on == cn.AI_id:
                ...
                df = pd.merge(hourly, cw,
                              ...
                              how="right").dropna()
                df.drop([f"{cn.DATE_COL}_temp", f"{cn.DATE_COL}_x"], axis=1, inplace=True)
            ...
            if self.on == cn.AI_id:
                ...
                if scaling != None:
                    if scaling == "min_max":
                        ...
                        raise NotImplementedError("MinMax scaling is the only one implemented at the moment.")
                ...
                for datetime_type in input_temporal_granularities:
                    df[datetime_type] = dt_mapper(df[cn.DATE_COL], datetime_type)
                
                dates = df[cn.DATE_COL]
                df = df.drop([cn.DATE_COL], axis=1)

                assert (train_mass + test_mass) == 10, "Train and test split is not consistent."
